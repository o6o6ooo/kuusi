...
struct AppMessage: Equatable {
	...
	init(
		_ id: ID,
		_ tone: Tone,
		autoClearAfter: TimeInterval? = defaultAutoClearInterval
	) {
		self.id = id
		self.tone = tone
		self.text = id.text
...
extension AppMessage.ID {
	var text: String {
		...
		case .googlePhotosPickerReturnedInvalidLink:
			return String(
				localized: "message.google_photos_picker_returned_invalid_link"
			)
		case .googlePhotosPickerTimedOut:
		...
		case .googleSignInReturnedIncompleteAccount:
			return String(
				localized: "message.google_sign_in_returned_incomplete_account"
			)
		case .googleSignInReturnedInvalidToken:
		...
		case .groupMemberLimitReached(let maxMembers):
			return String(
				format: String(localized: "message.group_member_limit_reached"),
				maxMembers
			)
		case .groupCreated:
		...
		case .photosImportedFromGooglePhotos(let count):
			return String(
				format: String(localized: "message.photos_imported_from_google_photos"),
				count
			)
		case .pleaseSignInFirst:
		...
		case .groupLimitReached(let title, let maxGroups):
			return String(
				format: String(localized: "message.group_limit_reached"),
				title,
				maxGroups
			)
		}
...
final class AppToastCenter: ObservableObject {
	...
	func present(
		...
		clearSource: (@MainActor @Sendable () -> Void)? = nil
	) {
		clearTask?.cancel()
	...
	func body(content: Content) -> some View {
		content
			.onAppear {
				...
				toastCenter.present(
					message,
					clearSource: { @MainActor @Sendable in
						clear()
					}
				)
			}
...
private struct AppToastHost: View {
	...
	var body: some View {
		GeometryReader { proxy in
			if toastCenter.isActiveHost(hostID),
				let message = toastCenter.currentMessage
			{
				HStack(spacing: 10) {
		...
		.animation(
			...
			value: toastCenter.currentMessage
		)
		.onAppear {
...
extension AppMessage.Tone {
	...
	fileprivate func symbolColor(for colorScheme: ColorScheme) -> Color {
		switch self {
		case .success:
