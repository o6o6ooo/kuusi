...
enum AppTelemetry {
	...
	private static func appVersion(from bundle: Bundle) -> String {
		let shortVersion =
			...
			as? String
		let build =
			bundle.object(forInfoDictionaryKey: kCFBundleVersionKey as String)
			as? String

		switch (shortVersion, build) {
		case (.some(let shortVersion), .some(let build))
		where !shortVersion.isEmpty && !build.isEmpty:
			return "\(shortVersion) (\(build))"
		case (.some(let shortVersion), _) where !shortVersion.isEmpty:
			return shortVersion
		case (_, .some(let build)) where !build.isEmpty:
			return build
		default:
			return "unknown"
		}
