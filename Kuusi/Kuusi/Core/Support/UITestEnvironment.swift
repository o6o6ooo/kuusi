...
enum UITestEnvironment {
    ...
    static func makePhotoCollectionFeedService() -> UITestPhotoCollectionFeedService? {
        guard isRunningUITests else { return nil }
...
final class UITestGroupService: SettingsGroupsServicing {
    ...
    func loadMemberPreviews(groupID: String, limit: Int?) async throws -> [GroupMemberPreview] {
        let members = UITestEnvironment.groups.first(where: { $0.id == groupID })?.members ?? []
        if let limit {
    ...
    func createGroup(groupName: String, ownerUID: String) async throws -> GroupSummary {
        GroupSummary(id: "ui-test-created-group", name: groupName, ownerUID: ownerUID, members: [], totalMemberCount: 1)
    }
    ...
    func joinGroup(inviteToken: String) async throws -> JoinGroupResult {
        JoinGroupResult(
            group: GroupSummary(id: "ui-test-joined-group", name: "Joined", ownerUID: "ui-test-owner", members: [], totalMemberCount: 1),
            didJoin: true
        )
    }
...
final class UITestPhotoCollectionFeedService: PhotoCollectionFeedServicing {
    func fetchRecentPhotoBatch(
        ...
        return RecentPhotoFetchResult(
            ...
            nextCursor: nil,
            favouriteIDs: favouriteIDs ?? Set(photos.filter { $0.isFavourite }.map(\.id))
        )
    }
