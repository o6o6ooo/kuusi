...
extension CachedRemoteImageView
...
private actor FeedImageCache {
	...
	private init() {
		...
		cacheDirectoryURL = cachesDirectory.appendingPathComponent(
			"feed-storage-image-cache",
			isDirectory: true
		)
	...
	private func loadDiskImage(for cacheKey: String) -> (
	...
	private static func cleanUpStaleDiskImages(
		...
		for fileURL in files where fileURL.pathExtension == "imgcache" {
			guard let lastUsedAt = lastUsedDate(for: fileURL), lastUsedAt < cutoff
			else {
				continue
			}
			try? fileManager.removeItem(at: fileURL)
	...
	private func fileURL(for cacheKey: String) -> URL {
		...
		return cacheDirectoryURL.appendingPathComponent(fileName)
			.appendingPathExtension("imgcache")
	}
}
