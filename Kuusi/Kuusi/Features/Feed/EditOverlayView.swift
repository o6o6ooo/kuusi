...
struct EditOverlayView: View {
	...
	let photo: FeedPhoto
	let onSave:
		@MainActor (_ update: FeedPhotoMetadataUpdate) async -> Result<
			Void, FeedEditError
		>
	...
	init(
		photo: FeedPhoto,
		onSave:
			@escaping @MainActor (_ update: FeedPhotoMetadataUpdate) async -> Result<
				Void, FeedEditError
			>
	) {
		self.photo = photo
	...
	private var photoImageSource: FeedImageSource? {
		if let thumbnailStoragePath = photo.thumbnailStoragePath,
			!thumbnailStoragePath.isEmpty
		{
			return .storagePath(thumbnailStoragePath)
		}

		if let previewStoragePath = photo.previewStoragePath,
			!previewStoragePath.isEmpty
		{
