...
private final class FeedNativeAdViewModel: NSObject, ObservableObject {
	...
	func loadIfNeeded(canLoadAds: Bool) {
		guard canLoadAds, nativeAd == nil, !isLoading, !hasFailedToLoad else {
			return
		}
...
private struct NativeAdRepresentable: UIViewRepresentable {
	...
	func makeUIView(context: Context) -> NativeAdView {
		...
		badgeLabel.font = scaledSystemFont(
			textStyle: .caption2,
			size: 11,
			weight: .semibold
		)
		...
		headlineLabel.font = scaledSystemFont(
			textStyle: .caption1,
			size: 13,
			weight: .semibold
		)
		...
		advertiserLabel.font = scaledSystemFont(
			textStyle: .caption2,
			size: 11,
			weight: .medium
		)
		...
		advertiserLabel.textColor = UIColor(
			AppTheme.primaryText(for: colorScheme).opacity(0.68)
		)
		...
		callToActionButton.titleLabel?.font = .preferredFont(
		...
		callToActionConfiguration.baseForegroundColor = UIColor(
		...
		NSLayoutConstraint.activate([
			...
			headlineLabel.bottomAnchor.constraint(
				lessThanOrEqualTo: callToActionButton.topAnchor,
				constant: -6
			),
