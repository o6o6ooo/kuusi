...
struct FeedView: View {
	...
	private var currentGroupName: String {
		groupStore.groups.first(where: { $0.id == groupStore.selectedGroupID })?
			.name ?? "Feed"
	}
	...
	var body: some View {
		NavigationStack {
			...
			.sheet(isPresented: $isUploadOverlayPresented) {
				UploadOverlayView(
					...
					onUploadCompleted: { uploadedPhotos in
						profileViewModel.addUploadedUsage(
							uploadedPhotos.reduce(0) { $0 + ($1.sizeMB ?? 0) }
						)
						if let uploadedGroupID = uploadedPhotos.first?.groupID {
							...
							photoCollection.replaceWithUploadedPhotosPendingReload(
								uploadedPhotos
							)
							Task {
								await photoCollection.reloadPhotosFromSource(
									limit: feedPageSize
								)
							}
						} else {
			...
			.onChange(of: displayedPhotos.map(\.id)) { _, ids in
				guard let selectedPhotoID, !ids.contains(selectedPhotoID) else {
					return
				}
	...
	private var feedSubtitle: String {
		if isFavouritesFilterEnabled, let selectedHashtag {
			return String(
				format: String(localized: "feed.subtitle.favourites_in_hashtag"),
				selectedHashtag
			)
		}
		...
		return String(
			format: String(localized: "feed.subtitle.photo_count"),
			photoCollection.currentGroupPhotoCount
		)
	}
	...
	private var emptyStateDescription: String {
		if isFavouritesFilterEnabled && selectedHashtag != nil {
			return String(
				localized: "feed.empty.no_favourites_for_hashtag.description"
			)
		}
		if isFavouritesFilterEnabled {
	...
	private func glassUnavailableView(
		title: String,
		systemImage: String,
		description: String
	) -> some View {
			...
			trackingAuthorizationStatus = await withCheckedContinuation {
				(
					continuation: CheckedContinuation<
						ATTrackingManager.AuthorizationStatus, Never
					>
				) in
	...
	private func toggleFavourite(_ photo: FeedPhoto) async {
		...
		do {
			...
			try await feedService.setFavourite(
				photoID: photo.id,
				userID: uid,
			...
			feedMessage =
				newValue
				? AppMessage(.addedToFavourites, .success)
				: AppMessage(.removedFromFavourites, .success)
		} catch {
	...
	private func savePhotoEdits(photo: FeedPhoto, update: FeedPhotoMetadataUpdate)
		async -> Result<Void, FeedEditError>
	{
		guard !editingPhotoIDs.contains(photo.id) else {
			return .failure(
				FeedEditError(
					toastMessage: AppMessage(.photoAlreadyBeingUpdated, .error)
				)
			)
		}
		guard currentUserID != nil else {
			return .failure(
				FeedEditError(toastMessage: AppMessage(.pleaseSignInFirst, .error))
			)
		}
		...
		do {
			...
			return .failure(
				FeedEditError(toastMessage: AppMessage(.failedToUpdatePhoto, .error))
			)
		}
	}
}
