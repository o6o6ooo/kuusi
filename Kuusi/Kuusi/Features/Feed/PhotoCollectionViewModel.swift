...
final class PhotoCollectionViewModel: ObservableObject {
	...
	func loadInitial(groups: [GroupSummary], selectedGroupID: String?, limit: Int)
		async
	{
		...
		photosByGroupID = loadCachedPhotos(
			for: uid,
			validGroupIDs: Set(groups.map(\.id))
		)
		if let favouriteIDs {
			photosByGroupID = Self.applyFavouriteIDs(
				favouriteIDs,
				to: photosByGroupID
			)
		...
		availableHashtagsByGroupID = photosByGroupID.mapValues(
			Self.makeAvailableHashtags(from:)
		)
		photoCountsByGroupID = loadCachedPhotoCounts(
			for: uid,
			validGroupIDs: Set(groups.map(\.id))
		)
		hasMorePhotosByGroupID = photosByGroupID.mapValues { !$0.isEmpty }
		nextCursorByGroupID = photosByGroupID.compactMapValues(
	...
	func syncGroups(_ groups: [GroupSummary], selectedGroupID: String?) {
		...
		} else if let current = self.selectedGroupID,
			groups.contains(where: { $0.id == current })
		{
		...
		availableHashtagsByGroupID = availableHashtagsByGroupID.filter {
			validGroupIDs.contains($0.key)
		}
		photoCountsByGroupID = photoCountsByGroupID.filter {
			validGroupIDs.contains($0.key)
		}
		nextCursorByGroupID = nextCursorByGroupID.filter {
			validGroupIDs.contains($0.key)
		}
		hasMorePhotosByGroupID = hasMorePhotosByGroupID.filter {
			validGroupIDs.contains($0.key)
		}
		removedPhotoIDsByGroupID = removedPhotoIDsByGroupID.filter {
			validGroupIDs.contains($0.key)
		}
		if !photosByGroupID.isEmpty {
	...
	func replacePhoto(_ updatedPhoto: FeedPhoto) {
		...
		guard var cachedPhotos = photosByGroupID[selectedGroupID],
			let index = cachedPhotos.firstIndex(where: { $0.id == updatedPhoto.id })
		else {
			return
		}
		...
		availableHashtagsByGroupID[selectedGroupID] = Self.makeAvailableHashtags(
			from: cachedPhotos
		)
	...
	func prependUploadedPhotos(_ uploadedPhotos: [FeedPhoto]) {
		...
		for groupID in Set(uploadedPhotos.compactMap(\.groupID)) {
			...
			photosByGroupID[groupID] = Self.mergeFreshPhotos(
				newPhotos,
				withExistingPhotos: existingPhotos
			)
			.sorted(by: Self.photosAreOrderedBefore)
			availableHashtagsByGroupID[groupID] = Self.makeAvailableHashtags(
				from: photosByGroupID[groupID] ?? []
			)
			incrementPhotoCount(for: groupID, by: newPhotos.count)
			nextCursorByGroupID[groupID] = Self.makeNextCursor(
				from: photosByGroupID[groupID] ?? []
			)
	...
	private func fetchPhotosForSelectedGroup(
		...
		do {
			...
			let presentedPhotos = Self.applyFavouriteIDs(
				result.favouriteIDs,
				to: mergedPhotos
			)
			photosByGroupID[selectedGroupID] = presentedPhotos
			availableHashtagsByGroupID[selectedGroupID] = Self.makeAvailableHashtags(
				from: presentedPhotos
			)
	...
	private func loadCachedPhotos(for uid: String, validGroupIDs: Set<String>)
		-> [String: [FeedPhoto]]
	{
		Self.cacheLock.lock()
		defer { Self.cacheLock.unlock() }
		...
		} else if let data = Self.defaults.data(
			forKey: Self.photoCacheKey(for: uid)
		),
			let decoded = try? JSONDecoder().decode(
				[String: [CachedFeedPhoto]].self,
				from: data
			)
		{
	...
	private func loadCachedPhotoCounts(
		for uid: String,
	...
	private func loadCachedFavouriteIDs(for uid: String) -> Set<String>? {
		...
		guard
			let data = Self.defaults.data(
				forKey: Self.favouriteIDsCacheKey(for: uid)
			),
	...
	private func refreshPhotoCountFromSource(groupID: String, uid: String) async {
		guard let count = try? await feedService.fetchPhotoCount(groupID: groupID)
		else {
			return
		}
	...
	private static func makeAvailableHashtags(from photos: [FeedPhoto])
		-> [String]
	{
		var seen = Set<String>()
		var ordered: [String] = []
	...
	private static func mergePhotos(
		_ existingPhotos: [FeedPhoto],
		with newPhotos: [FeedPhoto]
	) -> [FeedPhoto] {
	...
	private static func applyFavouriteIDs(
		_ favouriteIDs: Set<String>,
		to photosByGroupID: [String: [FeedPhoto]]
	) -> [String: [FeedPhoto]] {
	...
	private static func makeFavouriteIDs(
		from photosByGroupID: [String: [FeedPhoto]]
	) -> Set<String> {
		Set(
			photosByGroupID.values.flatMap { photos in
	...
	private static func mergeFreshPhotos(
		_ freshPhotos: [FeedPhoto],
		withExistingPhotos existingPhotos: [FeedPhoto]
	) -> [FeedPhoto] {
	...
	private static func makeNextCursor(from photos: [FeedPhoto])
		-> FeedPageCursor?
	{
		guard let lastPhoto = photos.last, let date = lastPhoto.date else {
			return nil
		}
		return FeedPageCursor(date: date, documentID: lastPhoto.id)
	}

	private static func photosAreOrderedBefore(_ lhs: FeedPhoto, _ rhs: FeedPhoto)
		-> Bool
	{
