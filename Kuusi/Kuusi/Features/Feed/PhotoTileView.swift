		...
		Group {
			if let user = viewModel.user {
				Text(user.icon)
					...
					.accessibilityLabel(
						String(
							format: String(localized: "photo.posted_by_accessibility"),
							user.name
						)
					)
			} else {
...
private struct ZoomablePreviewImageView<Placeholder: View>: View {
	...
	private func offset(
		...
		return CGSize(
			width: anchorOffset.x * (1 - scaleChange) + committedOffset.width
				* scaleChange,
			height: anchorOffset.y * (1 - scaleChange) + committedOffset.height
				* scaleChange
		)
	}

	private func clampedOffset(_ value: CGSize, scale: CGFloat, size: CGSize)
		-> CGSize
	{
...
struct PhotoTileView: View {
	...
	var body: some View {
		...
		.scaleEffect(1)
		.shadow(
			color: .black.opacity(isExpanded ? 0.1 : 0.08),
			radius: isExpanded ? 10 : 8,
			x: 0,
			y: isExpanded ? 5 : 4
		)
		.onTapGesture {
		...
		.contextMenu {
			...
			Button {
				...
				Label(
					photo.isFavourite
						? String(localized: "photo.menu.remove_from_favourites")
						: String(localized: "photo.menu.add_to_favourites"),
	...
	private var expandedMetaOverlay: some View {
		VStack(alignment: .leading, spacing: 10) {
			...
			HStack(alignment: .center, spacing: 8) {
				...
				Text(
					photo.date.map { Self.dateFormatter.string(from: $0) }
						?? String(localized: "photo.shared_memory")
				)
				.font(.subheadline.weight(.medium))
