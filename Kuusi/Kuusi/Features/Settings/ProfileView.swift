			...
			VStack(spacing: 14) {
				Menu {
					...
					ZStack {
						Color.clear
							.glassEffect(
								.regular.tint(Color(hex: viewModel.bgColour)),
								in: Circle()
							)
		...
		VStack(alignment: .leading, spacing: 12) {
			...
			HStack(alignment: .center, spacing: 12) {
				VStack(alignment: .leading, spacing: 4) {
					Text(
						viewModel.isGoogleLinked
							? String(
								format: String(localized: "profile.google_photos.connected_as"),
								viewModel.googleLinkedEmail
							) : String(localized: "profile.google_photos.not_connected")
					)
				...
				if viewModel.isGoogleLinked {
					...
					.buttonStyle(
						.appPrimaryCapsule(
							isLoading: viewModel.isGoogleAccountActionInFlight
						)
					)
