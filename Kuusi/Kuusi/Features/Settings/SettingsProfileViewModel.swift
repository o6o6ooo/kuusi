...
final class SettingsProfileViewModel: ObservableObject {
	...
	convenience init() {
			...
			if let userService = UITestEnvironment.makeProfileUserService() {
				self.init(
					...
					linkedAccountProvider: {
						GoogleLinkedAccount(email: "", isLinked: false)
					},
		...
		self.init(
			...
			linkedAccountProvider: {
				googleAccountService.currentLinkedAccount(for: Auth.auth().currentUser)
			},
			topViewControllerProvider: { UIApplication.topViewController() }
		)
	}
	...
	func loadProfile() async {
		...
		do {
			guard let user = try await userService.fetchCachedUser(uid: uid) else {
				return
			}
			name = user.name
			icon = user.icon
