		...
		NavigationStack {
			...
			.task {
				if !hasLoaded {
					...
					groupsViewModel.updateCurrentPlan(
						subscriptionStore.isPremiumActive ? .premium : .free
					)
					hasLoaded = true
				}
			}
			...
			.sheet(isPresented: $groupsViewModel.isGroupQRCodeOverlayPresented) {
				if let payload = groupsViewModel.selectedGroupInvitePayload {
					GroupQRCodeOverlayView(
						groupName: groupsViewModel.selectedGroup?.name
							?? groupsViewModel.editableGroupName,
						payload: payload
					)
...
private struct DeleteAccountReauthenticationView: View {
	...
	private func handleAuthorizationResult(
		...
		case .success(let authorization):
			...
			Task {
				onFinished()
				await appState.reauthenticateWithAppleAndDelete(
