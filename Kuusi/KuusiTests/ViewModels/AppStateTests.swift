...
struct AppStateTests {
	...
	@Test
	func reauthenticateWithAppleAndDeleteDeletesAccountWithoutRoutingToSignedIn()
		async
	{
		...
		let didDelete = await appState.reauthenticateWithAppleAndDelete(
	...
	@Test
	func reauthenticateWithAppleAndDeleteDoesNotDeleteWhenReauthenticationFails()
		async
	{
		let authService = AppleAuthServiceSpy(
			reauthenticationError: AuthServiceError.missingCurrentUser
		)
		let accountDeletionService = AccountDeletionServiceSpy()
		let appState = AppState(
			launchArguments: [],
	...
	@Test
	func
		reauthenticateWithAppleAndDeleteShowsRecentLoginMessageWhenAuthRequiresIt()
		async
	{
		let authService = AppleAuthServiceSpy(
			reauthenticationError: recentLoginError()
		)
		let accountDeletionService = AccountDeletionServiceSpy()
		let appState = AppState(
			launchArguments: [],
	...
	{
		let accountDeletionService = AccountDeletionServiceSpy(
			deleteAuthCurrentUserError: recentLoginError()
		)
		let notificationService = NotificationServiceSpy()
		let appState = AppState(
			launchArguments: [],
	...
	func clearToastMessageClearsCurrentToast() {
		...
		func refreshDebugAccountsReloadsAccountsAndSelection() {
			let loader = DebugAccountsLoaderSpy(accounts: [
				debugAccount(id: "debug-1")
			])
			let appState = AppState(
				launchArguments: [],
				biometricAuthService: BiometricAuthServiceSpy(result: true),
...
private final class AppleAuthServiceSpy: AppleAuthServicing {
	...
	func reauthenticateCurrentUser(payload: AppleSignInPayload) async throws
		-> String
	{
...
private final class AccountDeletionServiceSpy: AccountDeletionServicing {
	...
	init(
		deleteCurrentUserDataError: Error? = nil,
		deleteAuthCurrentUserError: Error? = nil
	) {
