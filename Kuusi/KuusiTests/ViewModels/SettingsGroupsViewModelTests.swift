...
struct SettingsGroupsViewModelTests {
	...
	func parserReadsInviteFromHttpsURL() {
		let inviteToken = GroupInvitePayloadParser.extractInviteToken(
			from: "https://kuusi.app/invite/Group-42"
		)
	...
	func createGroupRejectsPlanLimit() async {
		...
		#expect(
			viewModel.createStatusMessage?.id
				== .groupLimitReached(title: "Free", maxGroups: 3)
		)
	}
	...
	func createGroupAppendsSelectedGroupCachesAndShowsSuccess() async {
		...
		#expect(
			groupService.cachedGroupsAssignments.last?.groups.map(\.id) == [
				"created-group"
			]
		)
		#expect(
			groupService.loadMemberPreviewsCalls.contains {
				$0.groupID == "created-group" && $0.limit == 3
			}
		)
	}
	...
	func loadGroupsUsesCachedGroupsAndCachedPreviews() async {
		let groupService = SettingsGroupsServiceSpy()
		let cachedMember = GroupMemberPreview(
			id: "member-1",
			name: "Mia",
			icon: "🌸",
			bgColour: "#fff",
	...
	func refreshGroupsFetchesGroupsAndReloadsPreviews() async {
		let groupService = SettingsGroupsServiceSpy()
		let refreshedMember = GroupMemberPreview(
			id: "member-2",
			name: "Noah",
			icon: "🌿",
			bgColour: "#eee",
	...
	func presentMemberListFetchesAndCachesMembersWhenMissing() async {
		...
		#expect(viewModel.isMemberListPresented)
		#expect(
			viewModel.selectedGroupMembers.map(\.id) == ["member-1", "member-2"]
		)
		#expect(groupService.loadMemberPreviewsCalls.count == 1)
		...
		#expect(
			groupService.cachedMemberListAssignments.last?.members.map(\.id) == [
				"member-1", "member-2",
			]
		)
		#expect(
			viewModel.groups.first?.members.map(\.id) == ["member-1", "member-2"]
		)
	...
	func refreshSelectedGroupMembersFetchesCachesAndUpdatesPreview() async {
		...
		#expect(groupService.loadMemberPreviewsCalls.first?.limit == nil)
		#expect(
			viewModel.selectedGroupMembers.map(\.id) == [
		...
		#expect(
			viewModel.groups.first?.members.map(\.id) == [
	...
	func ownerLeaveConfirmationUsesTransferMessage() async {
		...
		let viewModel = makeViewModel(groupService: groupService)
		viewModel.groups = [
			makeGroup(id: "group-1", name: "Family", ownerUID: "user-1")
		]
		viewModel.selectedGroupID = "group-1"
		viewModel.selectedDestructiveAction = .leave

		#expect(
			viewModel.destructiveActionTitle
				== String(localized: "alert.leave_group.title")
		)
		#expect(
			viewModel.destructiveActionMessage
				== String(localized: "alert.leave_owned_group.message")
		)
		#expect(
			viewModel.destructiveActionButtonTitle
	...
	func leaveSelectedGroupRemovesCurrentGroupSelectsNextAndShowsSuccess() async {
		...
		#expect(
			groupService.cachedGroupsAssignments.last?.groups.map(\.id) == ["group-2"]
		)
		#expect(groupService.loadMemberPreviewsCalls.isEmpty)
	}
	...
	{
		let groupService = SettingsGroupsServiceSpy()
		let targetMember = GroupMemberPreview(
			id: "member-1",
			name: "Mia",
			icon: "🌸",
			bgColour: "#fff",
		...
		let remainingMember = GroupMemberPreview(
			id: "member-2",
			name: "Noah",
			icon: "🌿",
			bgColour: "#eee",
		...
		groupService.memberPreviewsByGroupID["group-1"] = [
			targetMember, remainingMember,
		]
		let viewModel = makeViewModel(groupService: groupService)
		viewModel.groups = [
			GroupSummary(
		...
		#expect(
			groupService.cachedGroupsAssignments.last?.groups.first?.totalMemberCount
				== 1
		)
		#expect(groupService.cachedMemberListAssignments.last?.groupID == "group-1")
		#expect(
	...
	private func makeGroup(id: String, name: String, ownerUID: String = "owner")
		-> GroupSummary
	{
		GroupSummary(
			id: id,
...
private final class SettingsGroupsServiceSpy: SettingsGroupsServicing {
	...
	var removeMemberCalls:
		[(groupID: String, memberUID: String, requesterUID: String)] = []
	...
	var cachedMemberListAssignments:
		[(groupID: String, members: [GroupMemberPreview])] = []

	func cachedGroups(for uid: String) -> [GroupSummary] {
		cachedGroupsAssignments.last(where: { $0.uid == uid })?.groups
			?? cachedGroups
	}
